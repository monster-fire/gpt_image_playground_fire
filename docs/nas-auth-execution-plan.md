...
状态：P0 已实现统一认证与 NAS 配置文件读写，并通过本地前后端、浏览器和 Docker 测试；NAS 收藏接口仍待实施。更新日期：2026-09-15。

### 本次落地状态

- `server/app.mjs` 提供登录、固定 168 小时会话、退出、配置 GET/PUT 和受保护代理；会话文件为 `/data/sessions.json`，密码变更会使旧会话失效。
- Docker 镜像采用 Node 22 + Nginx，无新增 npm 后端依赖。`APP_PASSWORD` 必填；HTTP 内网设 `COOKIE_SECURE=false`，HTTPS 设 `true`；反代域名用 `APP_ORIGIN` 指定。
- 使用 `compose.yaml` 部署时，`.env` 按 `.env.example` 配置；宿主端口 `PORT` 默认 11130，容器内部固定 80。挂载整个 `./config:/config` 和 `./data:/data`，不能只挂载 JSON 单文件，否则原子替换可能失败。
- 已有 `config/gpt-image-playground.json` 直接读取，设置修改写回同一文件；首次没有文件时进入未配置状态。`DEFAULT_API_URL` 不再用于把私有 JSON 注入 JS，已有文件需放到 `/config/gpt-image-playground.json` 或设置 `API_CONFIG_PATH`。
- 旧浏览器凭据仅为迁移暂留，不参与运行配置回退。登录后设置页可迁移，同 ID 保留 NAS 配置；保存并重新读取成功后清除旧副本。完整迁移预览/逐项冲突编辑与配置锁定策略仍属后续配置体验工作。
- 本批未实现收藏上传、SQLite、NAS 媒体删除，以下相关条目继续作为后续计划；当前收藏仍在浏览器。
- 当前接口为 `GET /api/auth/session`、`POST /api/auth/login`、`POST /api/auth/logout`、`GET/PUT /api/api-config`。写配置使用 `If-Match` 与 CSRF；版本冲突返回 409。后续细粒度 CRUD/批量接口以实际实施为准。

复验命令与逐项记录见 [P0 实施验证](product-experience-improvement-plan.md#6-本次交付边界)。本机 Docker 验证不代表已在用户 NAS 上部署。
...
| SESSION_TTL_HOURS=168 | 默认会话从登录成功起固定 7 天有效，不随访问自动续期。 |
...
### 7 天登录会话（2026-09-15 确认）

- 进入页面先检查会话；没有有效会话只显示密码登录页，校验完成前不挂载工作区、不加载私有配置、不发起生成。登录服务不可用时显示连接失败与重试入口，不能绕过认证。
- 密码由 Docker 启动环境变量 `APP_PASSWORD` 设置，服务端校验。浏览器不将密码或其哈希保存到 localStorage/IndexedDB，不用本地 `isLoggedIn` 标志作为授权依据。
- 登录成功生成不可预测的随机令牌，浏览器仅保存会话 Cookie：`HttpOnly; SameSite=Lax; Path=/; Max-Age=604800`，HTTPS 下加 `Secure`，不设置宽泛 Domain。服务端独立校验固定 `expiresAt`，客户端修改时间或存储不能延长会话。
- 正常关闭和重开浏览器仍可在有效期内免登；第 7 天到期后必须重新输入密码。清除 Cookie、主动退出或密码修改可使会话提前结束。不要按每次访问滑动续期。
- 会话记录保存到持久数据卷，只保存令牌摘要及必要的签发、到期、撤销信息；容器重启且密码未变时保留剩余有效期，不能重新计时。启动时检查密码版本，环境变量密码变更后撤销全部旧会话。版本标记不得存储明文密码或简单可猜测摘要，应使用服务端秘密参与的摘要或合适的密码派生方式。
- 退出登录在服务端撤销当前会话并删除 Cookie；跨标签页同步退出。其他设备保持独立会话，除非密码变更或管理员撤销全部会话。
- 已打开页面在到期时锁定工作区，切回前台与提交前重新校验会话；服务端每次请求都校验，不依赖前端定时器。过期/退出中止浏览器请求并清理内存密钥，保留本地原图、压缩缓存与草稿；再次登录后不自动重发生成请求。
- 登录接口设置失败限流及合理退避，写接口做 Origin/CSRF 校验。`/api-proxy/` 必须先鉴权再转发，并移除浏览器 Cookie 等本地认证信息，不把它们传给外部供应商。
- 公开内容只包含登录所需静态资源与不含秘密的应用代码。任何私有配置、媒体和生成代理必须在服务端保护；停止将含 API Key 的配置 JSON 注入匿名可下载的 JS。已公开过的供应商 Key 应轮换，登录不能撤回已泄露的密钥。
- 推荐通过 NAS 反向代理提供 HTTPS。用户当前的局域网 HTTP 访问只有显式配置 `COOKIE_SECURE=false` 才能携带 Cookie，但不提供传输加密；公网部署使用 HTTPS。Cookie 按主机而非端口隔离，使用独立域名与独立会话 Cookie 名，避免与 NAS 同主机其他应用混用。

此会话设计对应个人或家庭共享密码，不扩展为注册、角色权限或多用户数据隔离系统。共享设备仍需主动退出；应用登录不等于浏览器本地数据加密。

认证成功后才挂载实际工作区；退出或 NAS 明确返回会话失效时中止本页网络和上传、卸载私有 UI、清理内存配置和对象 URL。供应商返回的 401 不触发应用退出。本地数据库继续保留，登录入口不等同于本地磁盘加密。

NAS 提供的所有配置接口、收藏接口、媒体原图/缩略图、下载及可选 /api-proxy/ 统一鉴权。浏览器本地 IndexedDB 读取和本地下载不新增服务端鉴权调用。public/sw.js 排除私有路径，私有响应 Cache-Control: no-store，更新缓存版本并测试登出后的缓存行为。

### 统一鉴权范围与供应商凭据

应用登录会话与供应商 API Key 完全独立，完整请求矩阵见 [UX-17](product-experience-improvement-plan.md#ux-17网页与-nas-统一鉴权登录有效期-7-天)。应用鉴权只保护网页与 NAS 的交互：读取/修改 NAS 配置（包括读取 API Key）、收藏、媒体和代理入口。网页直连供应商时仅沿用供应商 API Key/SDK 认证，不附加本应用登录凭据或 CSRF token。

经过 NAS 代理的请求分为两跳：浏览器到 NAS 先验证应用会话；NAS 到供应商使用原供应商 API Key。不能因为代理请求带了 API Key 就跳过 NAS 会话校验，也不能把 NAS session 当作供应商 Bearer token。鉴权中间件通过 Cookie 识别会话，不覆盖供应商 Authorization；转发前去除 Cookie 和本地 CSRF 头。认证子请求只处理所需请求头，不转发大图片正文。

前端分开同源 NAS 客户端与供应商客户端。只有 NAS 的明确会话错误标记（例如 APP_SESSION_EXPIRED）触发登录失效；供应商 401，无论直连还是代理，都提示 API Key 问题并保留应用登录。网关要区分自身鉴权错误和上游响应，过滤可能混淆来源的上游同名标记。登录/会话检查和必要静态资源允许认证引导，其余 NAS 私有入口统一验证。

7 天登录控制的是应用工作区和 NAS 服务访问，不控制供应商 Key 自身的有效期。退出、到期或更换 APP_PASSWORD 不会撤销供应商 Key；已取得 Key 的人仍可能在其他客户端调用供应商，这与网页鉴权是不同边界。
...
阶段 A 优先独立交付，无需等待收藏上传完成；同时完成保护私有配置所必需的注入移除和鉴权读取，不能只上线密码弹窗而保留公开密钥或匿名代理。
...
18. 登录后关闭/重开浏览器，未到 168 小时仍有效；以服务端可控时钟验证第 168 小时到期，频繁使用不延长；篡改本地登录标志、客户端时间或过期 Cookie 均不能通过 API 鉴权。
19. 退出后旧令牌不能继续调用；修改 APP_PASSWORD 并重启后所有旧会话失效；密码不变且数据卷保留时容器重建维持剩余有效期。过期会话定期清理，不无限积累。
20. 未登录直接 POST /api-proxy/responses 被拒绝且上游没有收到请求；匿名抓取静态资源得不到密码或供应商 Key；外部供应商收不到本地会话 Cookie。
21. 到期时页面不再启动新请求，重登保留草稿和历史且不自动重发；多标签退出一致。验证 HTTPS 正常 Cookie、内网显式 HTTP 配置、登录限流、服务不可达与空密码启动失败。
22. 有效供应商 Key 不能绕过 NAS 会话鉴权；有效会话下供应商 Key 失效仅提示供应商认证错误，不退出应用。直连和代理两种路径均验证；上游收到原供应商认证头且不收到应用 Cookie/CSRF，鉴权子请求不携带图片正文。
